...
class DrunkardsOdometry(nn.Module):
    ...
    def initializer_scene_flow(self, image1, pose_2_1):
        ...
        T = SE3(data).view((batch_size, ht // 8, wd // 8))
        T_pose = SE3(data).view((batch_size, ht // 8, wd // 8))
    ...
    def predict_poses(self, image1, image2):
        ...
        pose = SE3.exp(pose).vec() # quaternions
    ...
    def forward(self, image1, image2, depth1, depth2, intrinsics, iters=12, train_mode=False, depth_scale_factor=None):
        """ Estimate optical flow between pair of frames """
        # Estimate relative camera pose to pass from cam1 to cam2
        ...
        T, T_pose = self.initializer_scene_flow(image1, pose_cnn)
        ae, coords0 = self.initializer(image1)
        ...
        for itr in range(iters):
            ...
            if train_mode:
                ...
                T_up_list.append(T_up.vec())
                pose_list.append(self.invert_pose(pose.vec().squeeze()))

        if train_mode:
            _, _, valid = pops.induced_flow(T_up, depth1, intrinsics)
            valid = valid > 0.5

            # Append the estimated camera pose by the initializer cnn to the last position
