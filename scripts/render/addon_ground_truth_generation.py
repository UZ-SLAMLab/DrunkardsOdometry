""" Script modified by David Recasens, University of Zaragoza."""
...
bl_info = {
        ...
        "author":"Joao Cartucho. Modified by David Recasens",
        "version":(1, 0),
        "blender":(2, 83, 4),
        "location":"PROPERTIES",
...
def get_camera_parameters_intrinsic(scene):
    ...
    print("sensor_width_in_mm: {}     sensor_height_in_mm: {}".format(sensor_width_in_mm, sensor_height_in_mm))
    ...
    f_x = f_in_mm * s_u
    f_y = f_in_mm * s_v
    c_x = resolution_x_in_px*scale / 2
    c_y = resolution_y_in_px*scale / 2
...
def save_data_to_npz(scene, is_stereo_activated,
    ...
    if is_stereo_activated:
        ...
        out_path1 = os.path.join(gt_dir_path, '{:010d}{}.npz'.format(scene.frame_current, suffix1))
        ...
        out_path0 = os.path.join(gt_dir_path, '{:010d}{}.npz'.format(scene.frame_current, suffix0))
    else:
        out_path0 = os.path.join(gt_dir_path, '{:010d}.npz'.format(scene.frame_current))
    out_dict_filtered0 = {k: v for k, v in out_dict0.items() if v is not None}
    np.savez_compressed(out_path0, **out_dict_filtered0)

def save_flow_to_npz(scene, opt_flw):    
    gt_dir_path = os.path.dirname(scene.render.filepath)    
    if not os.path.isdir(os.path.join(gt_dir_path, 'optical_flow')):
        os.mkdir(os.path.join(gt_dir_path, 'optical_flow'))
    out_path = os.path.join(gt_dir_path, 'optical_flow', '{:010d}.npz'.format(scene.frame_current))       
    np.savez_compressed(out_path, optical_flow=opt_flw.astype(np.float16))

def save_normal_to_npz(scene, normal_map):    
    gt_dir_path = os.path.dirname(scene.render.filepath)    
    if not os.path.isdir(os.path.join(gt_dir_path, 'normal')):
        os.mkdir(os.path.join(gt_dir_path, 'normal'))
    out_path = os.path.join(gt_dir_path, 'normal', '{:010d}.npz'.format(scene.frame_current))       
    np.savez_compressed(out_path, normal=normal_map.astype(np.float16))
...
def load_handler_after_rend_frame(scene): # TODO: not sure if this is the best place to put this function, should it be above the classes?
    ...
    if scene.vision_blender.bool_save_gt_data:
        ...
        if check_if_node_exists(scene.node_tree, 'output_vision_blender'):
            ...
            if vision_blender.bool_save_normals:
                if is_stereo_activated:
                    tmp_file_path1 = os.path.join(TMP_FILES_PATH, '{:010d}_Normal{}.exr'.format(scene.frame_current, suffix1))
                    normal1 = load_file_data_to_numpy(scene, tmp_file_path1, 'Normal')
                    tmp_file_path0 = os.path.join(TMP_FILES_PATH, '{:010d}_Normal{}.exr'.format(scene.frame_current, suffix0))
                else:
                    tmp_file_path0 = os.path.join(TMP_FILES_PATH, '{:010d}_Normal.exr'.format(scene.frame_current))
                normal0 = load_file_data_to_numpy(scene, tmp_file_path0, 'Normal')
            """ Depth + Disparity """
            if vision_blender.bool_save_depth:
                if is_stereo_activated:
                    tmp_file_path1 = os.path.join(TMP_FILES_PATH, '{:010d}_Depth{}.exr'.format(scene.frame_current, suffix1))
                    z1, disp1 = load_file_data_to_numpy(scene, tmp_file_path1, 'Depth')
                    tmp_file_path0 = os.path.join(TMP_FILES_PATH, '{:010d}_Depth{}.exr'.format(scene.frame_current, suffix0))
                else:
                    tmp_file_path0 = os.path.join(TMP_FILES_PATH, '{:010d}_Depth.exr'.format(scene.frame_current))
            ...
            if scene.render.engine == "CYCLES":
                ...
                if vision_blender.bool_save_segmentation_masks and check_any_obj_with_non_zero_index():
                    ...
                    if is_stereo_activated:
                        tmp_file_path1 = os.path.join(TMP_FILES_PATH, '{:010d}_Segmentation_Mask{}.exr'.format(scene.frame_current, suffix1))
                        seg_masks1 = load_file_data_to_numpy(scene, tmp_file_path1, 'Segmentation')
                        tmp_file_path0 = os.path.join(TMP_FILES_PATH, '{:010d}_Segmentation_Mask{}.exr'.format(scene.frame_current, suffix0))
                    else:
                        tmp_file_path0 = os.path.join(TMP_FILES_PATH, '{:010d}_Segmentation_Mask.exr'.format(scene.frame_current))
                    seg_masks0 = load_file_data_to_numpy(scene, tmp_file_path0, 'Segmentation')
                """ Optical flow - Forward -> from current to next frame"""
                if vision_blender.bool_save_opt_flow:
                    if is_stereo_activated:
                        tmp_file_path1 = os.path.join(TMP_FILES_PATH, '{:010d}_Optical_Flow{}.exr'.format(scene.frame_current, suffix1))
                        opt_flw1 = load_file_data_to_numpy(scene, tmp_file_path1, 'OptFlow')
                        tmp_file_path0 = os.path.join(TMP_FILES_PATH, '{:010d}_Optical_Flow{}.exr'.format(scene.frame_current, suffix0))
                    else:
                        tmp_file_path0 = os.path.join(TMP_FILES_PATH, '{:010d}_Optical_Flow.exr'.format(scene.frame_current))                    
                    opt_flw0 = load_file_data_to_numpy(scene, tmp_file_path0, 'OptFlow')
            # Optional step - delete the tmp output files
            clean_folder(TMP_FILES_PATH)
        """ Save data """
        save_flow_to_npz(scene, opt_flw0)
        save_normal_to_npz(scene, normal0)
