...
# Script to evaluate the Drunkard's Odometry in all the four levels of difficulty of the Drunkard's Dataset test scenes.
# As the Drunkard's Odometry model was trained with ground truth depth in metric scale, scale alignment is not needed, so SE(3) alignment.
