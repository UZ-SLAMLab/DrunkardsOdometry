...
# To compute the APTE of all the metrics, introduce these scale factors in eval_apte.py and execute it.
...
do    	
	...
	do
		...
		evo_ape tum "$root/$scene/$reference_method/pose_est_for_and_back.txt" "$root/$scene/$method/pose_est_for_and_back.txt" $align $correct_scale $plot $align_origin -va --save_results "$root/$scene/level$level_idx/$method/results_ate.zip"
	done
